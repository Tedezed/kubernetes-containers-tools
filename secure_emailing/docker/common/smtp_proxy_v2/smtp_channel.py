...
class SMTPChannel(smtpd.SMTPChannel):
    ...
    def found_terminator(self):
        ...
        if self.__state == self.COMMAND:
            ...
            if self.debug:
                self.logger.info("DATA: %s %s %s" % (self.__mailfrom, self.username, self.password))
                self.logger.info("DATA: %s" % self.credential_validator.validate_mailfrom(self.__mailfrom, self.username, self.password))

            # Control domain and address
            if self.credential_validator.validate_mailfrom(self.__mailfrom, self.username, self.password):
                ...
                self.push('500 Error: No allow send email with %s' % self.__mailfrom)
                raise ExitNow()