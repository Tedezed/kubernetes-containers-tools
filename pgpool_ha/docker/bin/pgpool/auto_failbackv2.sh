		...
		then
			...
			NODE_ID=$(echo "$INFO_NODE" | cut -d "|" -f 1)
	  		...
	  		then
	  			...
	  			if [ $MODE = "pgpool" ];
	  			NODE_MASTER_STATUS_PGPOOL=$(pcp_node_info -h 127.0.0.1 -U $PG_USERNAME -p $PCP_PORT -n $NODE_MASTER_ID -w | cut -d " " -f 3)
	  		...
	  		then
	  			...
	  			NODE_STANBY_STATUS_PGPOOL=$(pcp_node_info -h 127.0.0.1 -U $PG_USERNAME -p $PCP_PORT -n $NODE_STANBY_ID -w | cut -d " " -f 3)
	  		fi
